// Libraries
import React, {FC, useMemo, useContext, useRef} from 'react'
...
import {PlotResizer} from 'giraffe/dist/index'
...
import {SizedConfig} from '@influxdata/giraffe/dist/types'
...
const XYPlot: FC<Props> = ({
  ...
  const axesCanvasRef2 = useRef<HTMLCanvasElement>(null)
  const layerCanvasRef2 = useRef<HTMLCanvasElement>(null)
    ...
    <AutoSizer className="giraffe-autosizer">
      {({width, height}) => (
        <>
          ...
          />
          <PlotResizer
            axesCanvasRef={axesCanvasRef2}
            config={config as SizedConfig}
            height={height/2}
            layerCanvasRef={layerCanvasRef2}
            width={width}
          />
        </>
      )}
